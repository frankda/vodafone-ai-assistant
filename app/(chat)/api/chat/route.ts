...
export async function POST(request: Request) {
  ...
  if (!chat && session?.user && session.user.id) {
  ...
  const result = await streamText({
    ...
    // system: systemPrompt,
    system: `After receiving an initial response from the user, carefully assess whether additional questions are absolutely essential to retrieve tradie from vector database using embeddings. Only proceed with further inquiries if the available information is insufficient or ambiguous. When inquriy more information, use inquireInfo tool instead of answering the user's question directly.`,
    ...
    tools: {
      ...
      getInformation: {
        description: `get tradie information from your knowledge base to answer questions'`,
        parameters: z.object({
          question: z.string().describe('the users question'),
        }),
        execute: async ({ question }) => {
          console.log('getInformation');
          return 'Information getted'
        },
      },
      inquireInfo: {
        // description: 'After receiving an initial response from the user, carefully assess whether additional questions are absolutely essential to retrieve tradie from vector database using embeddings. Only proceed with further inquiries if the available information is insufficient or ambiguous',  
        description: 'When inquriy more information, use this tool to ask for more information',
        parameters: z.object({
          message: z.string().describe('The message to ask for information.'),
        }),
        execute: async () => {
          console.log('inquireInfo');
          const { object } = await generateObject({
            model: customModel(model.apiIdentifier),
            system: `when user asks for tradie, ask more info, As a construction consultant, your role is to provide proper tradies which can suit the user's needs.
